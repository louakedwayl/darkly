...
> **Note :** Darkly réutilise certains flags entre breaches (#2 = #7, #6 = #12). Les flags sont identiques mais les **méthodes d'exploitation sont différentes**. C'est la méthode qui compte pour la défense.
...
## #1 — SQL injection basic (`?page=member`)
...
## #2 — SQL injection avancée (`?page=searchimg`)
...
## #5 — XSS advanced (Reflected XSS via `<object>` data URI bypass)
...
**5. Méthode finale — data URI base64**
...
- `htmlspecialchars()` protège contre l'escape d'attribut mais pas contre les URI schemes dangereuses dans `data="..."` d'un `<object>`

### Différence avec #4

- **#4 Stored** : payload persisté en DB, exécuté à chaque visite
...
- **Whitelist d'URI schemes** : autoriser seulement `http://`, `https://`, chemins relatifs. Refuser `data:`, `javascript:`, `vbscript:`, etc.
- `htmlspecialchars()` insuffisant sur les attributs URL → `filter_var($url, FILTER_VALIDATE_URL)` + validation stricte
- **CSP** : `object-src 'none'` désactive `<object>` et `<embed>`
- **CSP** : `script-src 'self'` empêche scripts inline et data URI
- Cookies `HttpOnly`

---

## #6 — Cookies (Cookie Manipulation)
...
Remplacement du cookie `I_am_admin` par `b326b5062b2f0e69046810717534cb09` → flag affiché.

### Faille exploitée

- **A01:2021 — Broken Access Control**
- État d'autorisation stocké côté client sans intégrité cryptographique

### Impact

Élévation de privilège triviale. Aucune vérification serveur de l'intégrité du cookie. Tout utilisateur peut forger son statut admin.

### Contre-mesures

- Ne jamais stocker l'état d'autorisation côté client. Session ID opaque + rôle dans la session serveur.
...
**Flag :** `f2a29020ef3132e01dd61df97fd33ec8d7fcd1388cc9601e7db691d17d4d6188`
**Note :** Même flag que breach #2 — méthode différente.
**Dossier :** `07_Spoof/`

### Résumé

La page `?page=b7e44c7a...` (hash caché du footer) contient des indices noyés dans des commentaires HTML. Avec les bons headers HTTP forgés (Referer + User-Agent), le serveur affiche le flag.

### Méthode

**1. Identification des indices (view-source)**
...
Ces indices sont noyés dans plusieurs commentaires de Lorem Ipsum (obfuscation par bruit).
...
→ The flag is : f2a29020...
```

`-A` = User-Agent · `-e` = Referer · `-L` = follow redirects

### Faille exploitée

- **CWE-290 — Authentication Bypass by Spoofing**
- **CWE-345 — Insufficient Verification of Data Authenticity**
- `Referer` et `User-Agent` sont **entièrement contrôlés par le client** et trivialement falsifiables.

### Bénéfice

Tout ce qui vient du client est non-fiable, y compris les headers HTTP. Aucun header standard ne peut servir d'authentification.

### Impact

- Bypass de contrôles d'accès basés sur User-Agent
- Bypass de filtres anti-bot
...
- mTLS si validation de l'identité client requise
...
3 étapes enchaînées : disclosure du fichier `htpasswd` via directory listing → crack MD5 → auth bypass sur `/admin/`.

### Méthode

**1. Découverte du fichier**

- `robots.txt` → chemin `/whatever/`
...
> ⚠️ **Debug Hydra** : caractères spéciaux dans `F=` peuvent casser le matching. Vérifier avant de lancer :
> ```bash
...
- Top 1000 testé en < 10 secondes
- Combiné avec User Enumeration (#14) : ciblage précis des emails confirmés
...
- Politique mot de passe forte + blacklist top 10000
...
curl -X POST "http://127.0.0.1:8080/index.php?page=upload" \
  ...
  -F "Upload=Upload"
→ The flag is : 46910d9c... + /tmp/hack.php succesfully uploaded.
```
...
Webshell persistante → pivot vers compromission système complète.

### Contre-mesures

- **Whitelist stricte d'extensions** : `['jpg', 'png', 'gif', 'webp']`
- **Validation des magic bytes** (`finfo_file()`, jamais `$_FILES['type']`)
- **Re-encoding** : `imagecreatefrompng()` détruit tout PHP injecté
...
Sur cette même page on trouve aussi les indices Spoof (#7) — preuve que rien n'est jamais vraiment caché côté client.
...
- nginx `deny all` sur les fichiers de dev/backup
- Suppression des fichiers de backup en production

---

## #13 — Survey (Value tampering)
...
Le formulaire de vote `?page=survey` valide les valeurs **uniquement côté client** (`<select>` 1-10). Côté serveur aucune validation de la plage. Envoi d'un POST avec `valeur=999` → accepté → flag exposé.
...
→ The flag is 03a944b4...
```

La moyenne du sujet 2 explose à `4218.19` — preuve que l'input hors-plage a été accepté.
...
- Manipulation des moyennes
- Pollution des données
- DoS applicatif si calcul de moyenne plante avec valeurs extrêmes
- En e-commerce : compromission de la réputation produit/vendeur
...
- Prepared statements + types stricts (`PDO::PARAM_INT`)
- Ne jamais se reposer sur `<select>` ou `maxlength` HTML — UX, pas sécurité

---

## #14 — Recover (User Enumeration)
...
# Bonus (5) — XSS / SQL / Redirect / Bruteforce / robots.txt understanding

---

## XSS understanding

- **Stored** (persisté DB, touche tous les visiteurs) vs **Reflected** (lien URL, victime ciblée) vs **DOM-based** (manipulation côté client sans serveur)
- Contextes d'injection : HTML, attribut, JS inline, CSS, URL — `htmlspecialchars()` ne couvre que le contexte HTML
...
- Stored procedures vulnérables si elles font du concat dynamique en interne
...
- Open redirect comme stepping stone OAuth/SAML (`redirect_uri` mal validé)
- DOM-based open redirect (`window.location = userInput`)
- Dangers de la regex sur les paths : contournements par `//`, `@`, encodage URL
...
- Disclosure de chemins sensibles (Donald Trump campaign 2016, Verizon 2017)
- Bots malveillants ignorent `Disallow` — seul Google Bot le respecte
- Alternative correcte : authentification + `sitemap.xml` pour les pages publiques

---

# Leçons techniques cumulées

1. **Reconnaissance d'abord** : `whatweb`, `robots.txt`, Ctrl+U → toujours avant tout payload.
2. **Deux routing parallèles** : `?page=` (PHP) vs `/admin/` (filesystem nginx). Tester les deux.
3. **MD5 et SHA-1 sont morts** pour le stockage de mots de passe. CrackStation casse en secondes.
4. **Security through obscurity ≠ sécurité** : #12 et #8.
...
8. **Multi-base = surface d'attaque démultipliée** : #13 (Survey cross-DB) et #9 (creds Bruteforce sans visiter signin).
...
11. **Input non quoté = pas besoin de fermer une string** : `Unknown column 'mot'` = input non quoté.
12. **Hex literal `0x...`** : équivalent strings sans guillemets. Bypass d'encodage URL et filtres WAF.
13. **Pattern Darkly Open Redirect** : logique inversée — flag s'affiche au bypass, pas à la redirection.
14. **Pattern Darkly "réponse divergente"** : flag exposé sur le comportement non-nominal (mail inexistant, site hors whitelist, valeur hors plage).
15. **Filtre WAF blacklist = défaite garantie** : bloque `php://` mais pas `../`. Toujours whitelist + `realpath()`.
16. **Pattern Darkly "feedback progressif"** : messages d'erreur changent selon proximité avec la cible (breach Include).
17. **Hydra http-get-form — pièges** : caractères spéciaux dans `F=` cassent le matching ; toujours vérifier la string avec curl avant de lancer ; `-t 1` pour debug.
18. **File upload — `Content-Type` trusted = RCE** : jamais `$_FILES['type']`. `finfo_file()` + re-encoding obligatoires. Combiné avec LFI = RCE directe.
19. **Validation client uniquement = rien** : `<select>`, `maxlength`, `pattern` HTML sont de l'UX. Toute valeur peut être envoyée via curl/Burp. Validation **toujours** côté serveur.
20. **Indices obfusqués par bruit** : sur #7, les indices critiques sont noyés dans du Lorem Ipsum. View-source systématique + lecture exhaustive de TOUS les commentaires HTML obligatoires.
21. **Découverte d'endpoints par taille de réponse** : `wc -c` sur `?page=X`. Toute taille différente de la home = page existante.
22. **Headers HTTP comme vecteur d'attaque** : `User-Agent`, `Referer`, `X-Forwarded-For` sont tous contrôlés client. Triviallement falsifiables avec `curl -A/-e/-H`. Ne peuvent jamais servir d'authentification.
23. **Réutilisation de flags par Darkly** : #2 = #7, #6 = #12. La grille intra demande la **méthode**, pas l'unicité des flags.
24. **Bypass de filtre par double-passage** : `str_replace('javascript:', '', $src)` une seule fois → `javascriptjavascript::` reconstruit le préfixe. Anti-pattern classique d'une blacklist non récursive.
25. **URI schemes alternatifs** : `data:text/html;base64,...` non bloqué par un filtre ciblant uniquement `javascript:`. Whitelist de schemes = seule protection valide.
26. **`htmlspecialchars()` insuffisant en contexte URL/attribut `data`** : ne protège pas contre les URI schemes dangereuses dans `<object data="...">`.
27. **CSP `object-src 'none'`** : désactive `<object>` et `<embed>` — élimine la surface XSS via ces éléments même si l'input est mal sanitizé.